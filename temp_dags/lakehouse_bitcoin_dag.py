"""
Lakehouse Bitcoin DAG - Loads bitcoin/crypto data from JSON files into Delta tables
...
dag = DAG(
    'lakehouse_bitcoin_load',
    default_args=default_args,
    description='Load bitcoin/crypto data from JSON to Delta using Lakehouse Engine',
    schedule_interval='0 */3 * * *',  # Every 3 hours
    catchup=False,
    tags=['lakehouse', 'bitcoin', 'crypto', 'delta', 'etl'],
)
...
def load_bitcoin_to_delta(**context):
    """Load bitcoin data to Delta using Lakehouse Engine"""
    try:
        from lakehouse.jobs import run_bitcoin_etl
        
        print("Starting Bitcoin/Crypto ETL pipeline...")
        result = run_bitcoin_etl()
        print(f"ETL pipeline completed successfully. Result: {result}")
        return result
    except Exception as e:
        print(f"Error in Bitcoin ETL: {str(e)}")
        raise
...
def verify_delta_table(**context):
    ...
    try:
        ...
        delta_path = "/opt/airflow/lakehouse/silver/bitcoin/market_data"
...
load_to_delta = PythonOperator(
    task_id='load_bitcoin_to_delta',
    python_callable=load_bitcoin_to_delta,
    dag=dag,
)
...
notify = BashOperator(
    ...
    bash_command='echo "Bitcoin data loaded to Delta Lake at $(date)"',
    dag=dag,
)
