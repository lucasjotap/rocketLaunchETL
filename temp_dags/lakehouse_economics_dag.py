"""
Lakehouse Economics DAG - Loads economics data from JSON files into Delta tables
...
dag = DAG(
    'lakehouse_economics_load',
    default_args=default_args,
    description='Load economics data from JSON to Delta using Lakehouse Engine',
    schedule_interval='0 11 * * 1',  # Every Monday at 11 AM
    catchup=False,
    tags=['lakehouse', 'economics', 'delta', 'etl'],
)
...
def load_economics_to_delta(**context):
    """Load economics data to Delta using Lakehouse Engine"""
    try:
        from lakehouse.jobs import run_economics_etl
        
        print("Starting Economics ETL pipeline...")
        result = run_economics_etl()
        print(f"ETL pipeline completed successfully. Result: {result}")
        return result
    except Exception as e:
        print(f"Error in Economics ETL: {str(e)}")
        raise
...
def verify_delta_table(**context):
    ...
    try:
        ...
        delta_path = "/opt/airflow/lakehouse/silver/economics/indicators"
...
load_to_delta = PythonOperator(
    task_id='load_economics_to_delta',
    python_callable=load_economics_to_delta,
    dag=dag,
)
...
notify = BashOperator(
    ...
    bash_command='echo "Economics data loaded to Delta Lake at $(date)"',
    dag=dag,
)
