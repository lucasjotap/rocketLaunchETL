"""
Lakehouse Finance DAG - Loads finance stock data from JSON files into Delta tables
...
dag = DAG(
    'lakehouse_finance_load',
    default_args=default_args,
    description='Load finance stock data from JSON to Delta using Lakehouse Engine',
    schedule_interval='0 10 * * 1-5',  # Every weekday at 10 AM (after data fetch)
    catchup=False,
    tags=['lakehouse', 'finance', 'delta', 'etl'],
)
...
def load_finance_to_delta(**context):
    """Load finance data to Delta using Lakehouse Engine"""
    try:
        from lakehouse.jobs import run_finance_stocks_etl
        
        print("Starting Finance Stocks ETL pipeline...")
        result = run_finance_stocks_etl()
        print(f"ETL pipeline completed successfully. Result: {result}")
        return result
    except Exception as e:
        print(f"Error in Finance Stocks ETL: {str(e)}")
        raise
...
def verify_delta_table(**context):
    ...
    try:
        ...
        delta_path = "/opt/airflow/lakehouse/silver/finance/stocks"
...
load_to_delta = PythonOperator(
    task_id='load_finance_to_delta',
    python_callable=load_finance_to_delta,
    dag=dag,
)
...
notify = BashOperator(
    ...
    bash_command='echo "Finance data loaded to Delta Lake at $(date)"',
    dag=dag,
)
